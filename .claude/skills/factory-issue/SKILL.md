...
4. Create the issue via the helper (honors FACTORY_DRY_RUN=1):
   - Feature: `bun tools/factory/issue-create.ts --type feature --title "<title>" --what "<what>" --why "<why>" [--constraints "<constraints>"]`
   - Bug: `bun tools/factory/issue-create.ts --type bug --title "<title>" --broken "<broken>" --expected "<expected>" --where "<where>"`
